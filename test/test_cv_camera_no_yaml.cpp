...
{
    ros::NodeHandle node;
    ros::Subscriber sub = node.subscribe("/cv_camera_no_yaml/image_raw", 1, &ImageCallback);
    ros::Rate r(10.0);
...
{
    ros::NodeHandle node;
    ros::Subscriber sub = node.subscribe("/cv_camera_no_yaml/camera_info", 1, &InfoCallback);
    ros::Rate r(10.0);
    while (sub.getNumPublishers() == 0)
    {
        r.sleep();
